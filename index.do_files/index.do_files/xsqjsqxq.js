        ...
        var page = {
            ...
            methods: {
              ...
              //显示出入证
                showCrz: function(){
                	var self = this;
                	var sqbh  = this.$route.query.SQBH;
                	//校验是否可查看
                	MOB_UTIL.doPost({ url: api.checkCanShow, params: {SQBH:sqbh} }).done(function(result) {
               		 if(result.data.flag){
               			var data = JSON.parse(result.data.data);
               		     self.$router.push({ 
		               		  ...
		               		  query: {
	                               XM: data.XM,
	                               YX:data.DWDM_DISPLAY,
	                               BJ:data.BJDM_DISPLAY,
	                               XH:data.XH,
	                               YXQ:data.YXQ,
	                               XSBH:data.XSBH,
	                               XB:data.XBDM_DISPLAY,
	                               MZ:data.MZDM_DISPLAY,
	                               NJ:data.XZNJ,
	                               ZY:data.ZYDM_DISPLAY,
	                               QJSY:data.QJSY,
	                               SSH:data.SSH
	                           }
               			  });
               		 }else{
               			 mintUI.MessageBox('提示', result.data.msg);
               		 }
               	 });
                }
            }
